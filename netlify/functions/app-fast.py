...
# Set FAST_START environment variable to avoid loading heavy ML models
os.environ['FAST_START'] = 'true'

try:
    # Import Flask app with fast start
    from app import app
    
    def handler(event, context):
        """
        Netlify Functions handler for Flask app (Fast Start Mode)
        """
        try:
            # Handle different path formats
            path = event.get('path', '/')
            if path.startswith('/.netlify/functions/app-fast'):
                path = path.replace('/.netlify/functions/app-fast', '') or '/'
            ...
            return {
                ...
                'body': f'{{"error": "Internal server error", "details": "{str(e)}", "traceback": "{error_details.replace(chr(34), chr(92)+chr(34))}"}}'
            }
    ...
    def handler(event, context):
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json'},
            'body': f'{{"error": "Failed to import Flask app", "details": "{str(import_error)}"}}'
        }